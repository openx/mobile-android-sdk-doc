...
The OpenX Mobile SDK supports the DeepLink+ starting from version **4.11**. The schema is supported for both types of ads - video and display.
...
Starting with version 4.11.0 the Android SDK supports deeplink+.
...
The only required parameter is `primaryUrl` and if there are no other parameters, the deeplink+ would be handled as standard deeplink URL: does nothing if the app is missing.
...
Check out the following examples from [Demo Application](https://storage.cloud.google.com/ox-cdn-prod-mobile/sdks/apollo/release/android/sdk/1.0.0/OpenX_Apollo_SDK_Android_Demo_1.0.0.zip) on the real device:
